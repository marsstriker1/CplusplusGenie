        ...
        for(int ksc = 0;ksc <wordNo;ksc++) //ksc = keyword string count
            {
                if(!foundword[ksc])
                {
                    ...
                    {
                    returninfo = string("The no of occurances of ")+keywords[ksc]+string(" for file: '")+filename[fc]+string("' is ")+ to_string(priority2[fc][ksc]);
                    searchEngineDi.add(returninfo);
                    }
                }
            }
   ...
   protected:
    ...
    public:
     void getInput(const char* syn1, const char* syn2)
     {
        /* cout<<"\n\nPLEASE ENTER A WORD ";
         cin>>synword1;
         cout<<"\nENTER THE SYNONYM FOR THE ENETERED WORD TO GET RESULT FOR FUTURE SEARCHES ";
         cin>>synword2;*/
         synword1 = syn1;
         //cout<<"\nSynword1: "<<synword1;
         synword2 = syn2;
...
void Search(const char* key,const char* dirpath)
{
    SearchEngine s;
    /*char dirname[20] ;
    string searchKeyword;   //user input to search
    cout<<"\nENTER DIRECTORY PATH[e.g. c:/dir1/dir2/] WHERE DATA FILES ARE TO BE READ ";
    cout<<"\nENTER 'd' TO USE DEFAULT DIRECTORY ";
    cin>>dirname;

    if(string(dirname) == string("d"))
    {*/
    ...
    if(s.isValid)
    {
    //s.openFileExternal(1);
    s.displayfile();
    //cout<<"\nENTER SEARCH TEXTS ";
    //getline(cin,dummy);
    //fflush(stdin);  //remove carriage key left by cin for getline
